...
public interface QuestionDAO extends CrudRepository<Question, Long> {
    ...
    @Query("SELECT p.quiz.questions FROM Progress p WHERE p.intern.id = (:internId) AND p.quiz.id = (:quizId)")
    ...
    @Query("SELECT q FROM Question q WHERE q.quiz.id = (:quizId) AND q.id NOT IN (:questionsIds)")
    ...
    @Query("SELECT qn FROM Question qn JOIN Quiz qz ON qn.quiz.id = qz.id JOIN Progress p ON p.quiz.id = qz.id LEFT JOIN Record r ON r.question.id = qn.id AND r.progress.id = p.id WHERE r.id IS NULL AND p.id = (:progressId)")
